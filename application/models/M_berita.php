...
class M_berita extends CI_Model {
	...
	function tampil_berita() {
		$hsl = $this->db->query("SELECT tb_berita.id, tb_berita.judul, tb_berita.tanggal, tb_berita.isi, tb_berita.tayang,tb_berita.slug,tb_berita.gambar,tb_kategori.kategori as kategori ,tb_user.nama , DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal1 FROM tb_berita INNER JOIN tb_kategori on tb_berita.kategoriid = tb_kategori.id INNER join tb_user on tb_berita.userid = tb_user.id ORDER BY tanggal DESC");
		return $hsl;
	}
	...
	function berita_perpage($offset, $limit) {
		$hsl = $this->db->query("SELECT tb_berita.id, tb_berita.judul, tb_berita.tanggal, tb_berita.isi, tb_berita.tayang,tb_berita.slug,tb_berita.gambar,tb_kategori.kategori as kategori ,tb_user.nama , DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal1 FROM tb_berita INNER JOIN tb_kategori on tb_berita.kategoriid = tb_kategori.id INNER join tb_user on tb_berita.userid = tb_user.id ORDER BY tanggal DESC limit $offset,$limit");
		return $hsl;
	}
	...
	function berita() {
		$hsl = $this->db->query("SELECT tb_berita.*,DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal1 FROM tb_berita ORDER BY tanggal DESC");
		return $hsl;
		// $hsl = $this->db->query("SELECT tb_berita.id, tb_berita.judul, tb_berita.tanggal, tb_berita.isi, tb_berita.tayang,tb_berita.slug,tb_berita.gambar,tb_kategori.kategori as kategori ,tb_user.nama , DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal FROM tb_berita INNER JOIN tb_kategori on tb_berita.kategoriid = tb_kategori.id INNER join tb_user on tb_berita.userid = tb_user.id ORDER BY id DESC");
		// return $hsl;
	}
	function get_berita1_by_kode($kode) {
		$hsl =	$query = $this->db->query("SELECT * from tb_berita JOIN tb_kategori ON tb_berita.kategoriid = tb_kategori.id JOIN tb_user ON tb_berita.userid WHERE tb_berita.id = '$kode' ");
		return $hsl;
	}

	function cari_berita($keyword) {
		$hsl = $this->db->query("SELECT tb_berita.id, tb_berita.judul, tb_berita.tanggal, tb_berita.isi, tb_berita.tayang,tb_berita.slug,tb_berita.gambar,tb_kategori.kategori as kategori ,tb_user.nama, DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal1 FROM tb_berita INNER JOIN tb_kategori on tb_berita.kategoriid = tb_kategori.id INNER join tb_user on tb_berita.userid = tb_user.id WHERE judul LIKE '%$keyword%' LIMIT 5");
		return $hsl;
	}
