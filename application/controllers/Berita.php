...
class Berita extends CI_Controller {
	...
	function kategori($kategori) {
		...
		$query = $this->db->query("SELECT * from tb_berita JOIN tb_kategori ON tb_berita.kategoriid = tb_kategori.id JOIN tb_user ON tb_berita.userid = tb_user.id WHERE tb_kategori.kategori LIKE '%$kategori%' ORDER BY tayang DESC LIMIT 5");
		if ($query->num_rows() > 0) {
			$x['data'] = $query;
			$x['agendaterbaru'] = $this->m_agenda->get_agenda_terbaru();
			$x['allkate'] = $this->m_kategori->get_all_kategori();
